...
func _set_attack(area_id: int, flag: Sprite2D, ui_army: _UIAttackArmy) -> void:
	var area := g_Scene.get_area(area_id)
	var country := area.country
	var flag_name := "flag_{0}.png".format([country.name])
	flag.texture = _ecImageTexture.from_ec_image_attr(g_GameRes.get_flag_image(flag_name))
