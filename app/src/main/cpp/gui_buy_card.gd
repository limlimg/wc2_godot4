...
func _get_sel_card() -> _CardDef:
	return _CObjectDef.instance().get_card_def(_selected_card)
...
func _can_buy_sel_card() -> bool:
	...
	if country == null:
		return false
	return country.can_buy_card(_CObjectDef.instance().get_card_def(_selected_card))
...
func reset_card_target() -> void:
	...
	if country != null and _targeting and _selected_card >= 0:
		...
		country.set_card_targets(_CObjectDef.instance().get_card_def(_selected_card))
...
func _on_gui_button_ok_pressed() -> void:
	...
	if card != null and _can_buy_sel_card():
		var country := g_GameManager.get_cur_country()
		var target := _CObjectDef.instance().get_card_target_type(card)
