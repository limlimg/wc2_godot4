...
func _set_conquest_player_country_id(value: String) -> void:
	_conquest_player_country_id = value


func _new_game(game_mode: int, map_id_minus_1: int, campaign: int, battle: int) -> void:
...
func _load_game(save_file_name: String) -> void:
	...
	if file.open(path, FileAccess.READ):
		...
		file.close()
		_game_mode = buffer.decode_s32(8)
		_map_id = buffer.decode_s32(16)
		_areas_enable = buffer.slice(20, buffer.find(0, 20)).get_string_from_ascii()
		_player_country_name[0] = buffer.slice(52, buffer.find(0, 52)).get_string_from_ascii()
		_player_country_name[1] = buffer.slice(60, buffer.find(0, 60)).get_string_from_ascii()
		_player_country_name[2] = buffer.slice(68, buffer.find(0, 68)).get_string_from_ascii()
		_player_country_name[3] = buffer.slice(76, buffer.find(0, 76)).get_string_from_ascii()
		_battle_file_name = buffer.slice(84, buffer.find(0, 84)).get_string_from_ascii()
		_campaign = buffer.decode_s32(172)
		_battle = buffer.decode_s32(176)
