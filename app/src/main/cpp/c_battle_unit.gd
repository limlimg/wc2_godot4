...
func attack() -> void:
	...
	_tween.tween_callback(func ():
		...
		match army_id:
			0:
				if _motion_index <= 1:
					g_SoundRes.play_char_se(_SndEffect.FIRE_WAV)
				else :
			...
			3:
				g_SoundRes.play_char_se(_SndEffect.ROCKET_WAV)
			4:
				g_SoundRes.play_char_se(_SndEffect.CANNON_WAV)
			5:
				g_SoundRes.play_char_se(_SndEffect.CANNON_WAV)
			6:
				g_SoundRes.play_char_se(_SndEffect.NAVAL_GUN_WAV)
			7:
				g_SoundRes.play_char_se(_SndEffect.NAVAL_GUN_WAV)
			8:
				g_SoundRes.play_char_se(_SndEffect.NAVAL_GUN_WAV)
