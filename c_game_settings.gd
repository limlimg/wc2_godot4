...
func load_settings() -> void:
	...
	if file.open(EC2dAppDelegate.get_document_path("settings.cfg"), FileAccess.READ):
...
func save_settings() -> void:
	...
	var file := _ecFile.new()
	if file.open(EC2dAppDelegate.get_document_path("settings.cfg"), FileAccess.WRITE):
