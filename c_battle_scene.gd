...
@export
var crater_texture_res: ecTextureRes

@export
var effect_texture_res: ecTextureRes
...
func _create_fort() -> void:
	var motions := CObjectDef.instance().get_unit_motions("fort", "")
	var res := motions.res
	var fort_res := ecTextureRes.new()
	var fort_lib := ecLibrary.new()
	if AppDelegate.g_content_scale_factor == 2.0 and fort_res.load_res(res + "_hd.xml", false):
...
func _create_units() -> void:
	...
	if army_id > 5:
		if AppDelegate.has_unit_motion(res, country_name):
			...
			res += "_ne"
	var unit_res := ecTextureRes.new()
	var unit_lib := ecLibrary.new()
	if AppDelegate.g_content_scale_factor == 2.0 and unit_res.load_res(res + "_hd.xml", false):
